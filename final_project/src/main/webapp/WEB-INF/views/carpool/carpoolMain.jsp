...
<body>
	<div id="wrapper">
		...
		<section class="section" style="padding-top: 0px; padding-bottom: 30px;">
			<div class="container">
				<div class="row">
	<!-- 강근님한테 질문 : 출발일, 작성일은 한번만 쓰이고 thead, 근데 tbody들은 for each로 반복되서 나올 것이고 section안에 넣으려고 했는데 테이블이 갈라져도 되나.. 
		그리고 각 요소를 클릭했을때 /carpoolRequest.do?carpoolNo=? 이렇게 가야하는데 이거 어떻게 적는지... -->
		...
		<table id="carpoolTable" class="tablesorter"
			...
			<c:forEach items="${list }" var="c">
				<section class="section" id="carpoolList"
					...
					<div class="container">
						<tbody>
						<tr style="cursor: pointer;" location.href='/carpoolRequest.do?'">
							<td>${c.dapartureDate } </td>
							<td>${c.regDate } </td>
							<td data-sort-method='none'>${c.tripType }</td>
							<td data-sort-method='none'>${c.regDate }</td>
							<td data-sort-method='none'>${c.reserved}/${c.capacity }</td>
							<td data-sort-method='none'>${c.departureRegion }</td>
							<td data-sort-method='none'>${c.departureDistrict }</td>
						</tr>
						</tbody>
	...
	<script>
		...
		cellName.innerHTML = 0;refresh.refresh();
