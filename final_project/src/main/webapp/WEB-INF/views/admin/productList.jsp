	        ...
	        <div>
	            ...
	            <div class="lesson-list list-wrapper">
                    <div class="newProduct-top list-top">
                        <div class="count">강습 전체 상품 <span>${lessonCount }</span></div>
                        <input type="hidden" value="2" class="lesson-product-type">
                        <table>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td>
	                                <span class="material-symbols-outlined more-detail">more_vert</span>
	                                <div class="list-detail-box" style="display:none" onblur="">
						                <div><a href="#" class="update-detail">상품 정보 수정</a>
						                <a href="#" class="delete-detail">상품 판매 중지</a></div>
					            	</div>
                                </td>
                            </tr>
                            </c:forEach>
                    ...
                    <div class="list-bottom">
                        <div>
                            <input type="submit" value="선택 상품 상태 변경" class="checkedUpdateLessonStatus btn bc1">
                        </div>
                    </div>
	            </div>
	            <div class="house-list list-wrapper" style="display:none;">
                    <div class="newProduct-top list-top">
                        <div class="count">숙박 전체 상품 <span>${houseCount }</span></div>
                        <input type="hidden" value="2" class="house-product-type">
                        <table>
                            <tr>
                            	<th><input type="checkbox" name="houseCheck" class="house-all-check chk"></th>
                                <th>상품명</th>
                                <th>판매자</th>
                                <th>숙박소 이름</th>
                                <th>지역</th>
                    ...
                    <div class="list-bottom">
                        <div>
                            <input type="submit" value="선택 상품 상태 변경" class="checkedUpdateHouseStatus btn bc1">
                        </div>
                    </div>
	            </div>
	        </div>
...
<script>
	...
	$(".checkedUpdateLessonStatus").on("click",function(){
	    ...
	    check.each(function(index,item){
	        ...
	        const lStatus = $(item).parent().parent().find('select').val();
	        status.push(lStatus);

	    });
	    
	    location.href="/checkedUpdateLessonStatus.do?no="+no.join("/")+"&status="+status.join("/");
	});
    
    /*숙박 상품 상태 변경*/
	//체크박스 선택상품
	$(".checkedUpdateHouseStatus").on("click",function(){
	    ...
	    check.each(function(index,item){
	        ...
	        const hStatus = $(item).parent().parent().find('select').val();
	        status.push(hStatus);

	    });
	    
	    location.href="/checkedUpdateHouseStatus.do?no="+no.join("/")+"&status="+status.join("/");
	});
